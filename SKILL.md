...
2. 找到 Grok 可执行文件后先运行 `--help`，以本机版本的参数为准。常见 Windows 路径为 `%USERPROFILE%\.grok\bin\grok.exe`，不可假定版本、模型或会话格式始终相同。对 1.0.4 及更新版本，额外运行 `grok doctor` 和 `grok inspect`；它们可能报告终端能力或旧配置 warning，这些提示要与会话失败分开判断。
...
优先使用 `--resume <id>`，不要用可能选中错误记录的 `--continue`。长提示写入 UTF-8 文本文件，避免 PowerShell 引号和编码损坏；每阶段保存独立的提示、stdout、stderr 与 debug 文件。只在有明确原因时使用 `--fork-session`、`--worktree` 或 `--restore-code`。
...
4. 只有在最终回复或 `turn_completed` 已出现、没有未完成工具调用，并且相隔约 10 秒的两次检查均无新活动时，才判断该阶段结束。脚本的 `wait` 只证明文件是否稳定，工具调用仍需读取事件与日志确认。1.0.4 的 headless 调用可能在主回答已经成功、退出码为 0 后，额外报告 session title、proactive bundle 或 telemetry 的后台同步 warning；应分别记录 stdout、stderr 和 debug，不要把这类 warning 单独判为阶段失败。若同时出现 `error decoding response body`、重试耗尽或主回答缺失，再按网络/代理故障处理。
...
Grok 1.0.4 的用户指南说明会在上下文窗口达到约 85% 时自动压缩；这不能代替用户指定的提前检查，但可以作为第二条安全线。手动 `/compact` 仍需在当前阶段结束后单独发送，并按上述证据核验。
...
- `inspect --session-id`：定位会话，汇总关键文件、上下文读数、压缩计数与最近事件线索。
- `wait --session-id --seconds 10`：比较两次文件快照，给出稳定性证据。
- 1.0.4 之后：先确认 `--version`、`--resume`、`--prompt-file`、`--output-format plain` 和 `--debug-file` 仍在 `--help` 中。可在新临时工作目录中用 `--single ... --output-format json` 获取 `sessionId`，再用原阶段命令恢复该 ID，做兼容性冒烟测试，避免触碰用户现有会话。
